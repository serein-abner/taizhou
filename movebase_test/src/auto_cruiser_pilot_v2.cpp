...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    for (size_t i = 0; i < goals.size(); i++)
    {
      ...
      //
      if (i >= 1 && if_returnhome)
      {
        break;
      }
    }
    ROS_INFO_STREAM("task loop break");
    //
    goal_pub.publish(goals[goals.size()-1]);
    while (ros::ok())
    {
      //
      try
      {
        ...
        geometry_msgs::Quaternion orientation = transformStamped.transform.rotation;
        //
        double dis = sqrt(pow(position.x - goals[goals.size() - 1].pose.position.x, 2) +
                          pow(position.y - goals[goals.size() - 1].pose.position.y, 2));
        double dis_pose = sqrt(pow(orientation.z - goals[goals.size() - 1].pose.orientation.z, 2) +
                               pow(orientation.w - goals[goals.size() - 1].pose.orientation.w, 2));
        // type 0 is path-pass point
        if (type_list[0] == 2)
        {
          if (dis <= 3)
