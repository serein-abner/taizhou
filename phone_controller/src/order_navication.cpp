...
{   
    ROS_INFO("Message arrived: Topic:/phoneControler/navication; Paylod:{ %s }",msg->data.c_str());
...
{
    ros::init(argc,argv,"order_navication");
    ROS_INFO("Node:order_navication,started");
    ros::NodeHandle h;
    ros::Subscriber startListener = h.subscribe<std_msgs::String>("/phoneControler/navication",10,startCallBack);
