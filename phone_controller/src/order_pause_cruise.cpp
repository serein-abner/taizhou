...
{
    ROS_INFO("Message arrived: Topic:/phoneControler/pauseCruise; Paylod:{ %s }", msg->data.c_str());
...
{
    ros::init(argc, argv, "order_pause_cruise");
    ROS_INFO("Node:order_pause_cruise,started");
    ros::NodeHandle h;
    ros::Subscriber startListener = h.subscribe<std_msgs::String>("/phoneControler/pauseCruise", 10, startCallBack);
