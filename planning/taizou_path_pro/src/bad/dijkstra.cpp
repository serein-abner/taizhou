...
void dijkstra::init()
{
    ...
    for (int i = 0; i < (points.size()); i++)
    {
        ...
        destination[sta_idx + points[i].size() - 1][sta_idx + points[i].size() * 2 - 1] = 0.1; // 端点自旋//此处为d1=>d2

        destination[sta_idx + points[i].size()][sta_idx] = 0.1; // 端点自旋//此处为a2=>a1

        destination[sta_idx + points[i].size() * 2 - 1][sta_idx + points[i].size() - 1] = 0.1; // 端点自旋//此处为d2=>d1

        destination[sta_idx][sta_idx + points[i].size()] = 0.1; // 端点自旋//此处为a1=>a2

        // if ((i + 1) < points.size())
        // {
        //     destination[sta_idx + points[i].size()][sta_idx + points[i].size() * 2] = 1; // 跨道//此处为a2=>h1
        //     std::cout << "\033[30m" << sta_idx + points[i].size() * 2 << "....... " << sta_idx + points[i].size() << "\033[0m"<<std::endl;
        // }
        // if ((i + 1)<points.size())
        //     destination[sta_idx + points[i].size() * 2 + points[i + 1].size() - 1][sta_idx + points[i].size() * 2 - 1] = 1; // 跨道//此处为k1=>d2
        // if ((i + 1) < points.size())
        // {
        //     int h2 = sta_idx + points[i].size() * 2 + points[i + 1].size();
        //     destination[h2][sta_idx] = 1; // 跨道//此处为h2=>a1
        // }
        // if ((i + 1) < points.size())
        // {
        //     int k2 = sta_idx + points[i].size() * 2 - 1 + points[i + 1].size()*2;
        //     destination[sta_idx + points[i].size() - 1][k2] = 1; // 跨道//此处为k2=>d1
        // }
...
{
    if(fail_to_find){
        std::vector<pcl::PointXYZ> empty_path;
        std::cout << "out put empty path" << std::endl;
        return empty_path;
    }
    else
        {
            std::cout << "out put good path" << std::endl;
            return path_points;
        }
}
